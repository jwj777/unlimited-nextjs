...
async function getArticlesBySlug(params) {
  try {
    const slug = params.slug;
    const response = await fetch(`https://unlimited-strapi-h4fgb.ondigitalocean.app/api/articles?filters[slug][$eq]=${slug}&populate[heroImage][populate]=*&populate[Sections][populate]=*`, { next: { revalidate: 60 } });
